...
from src.tools.buffer import prepare_replay_buffer, RewardNormalizer, StateNormalizer
from src.tools.utils import set_seed, wrap_env, soft_update, compute_mean_std, eval_actor
...
class TrainConfig:
    ...
    seed: int = 123  # Sets Gym, PyTorch and Numpy seeds
    ...
    max_timesteps: int = int(2.5e5)  # Max time steps to run environment
    checkpoints_path: Optional[str] = "checkpoints"
    critic_path: str = "critic_models" # Critic model load file name, "" doesn't load
    ...
    iql_tau: float = 0.6  # Coefficient for asymmetric loss
...
def pre_train_IQL(device="cuda:0", env="halfcheetah-medium-v2", seed=123, max_timesteps=int(1e5), eval_freq=int(1e4), iql_tau=0.7):
    #预训练阶段IQL的Q和V函数
    TensorBatch = List[torch.Tensor]
    EXP_ADV_MAX = 100.0
    LOG_STD_MIN = -5.0
    ...
    config  = TrainConfig(device=device, env=env, seed=seed, max_timesteps=max_timesteps, eval_freq=eval_freq, iql_tau=iql_tau)
    ...
    # Initialize actor
    IQL_trainer = ImplicitQLearning(**kwargs)

    if config.load_model != "":
        policy_file = Path(config.load_model)
        IQL_trainer.load_state_dict(torch.load(policy_file))
        # actor = trainer.actor
    ...
    for t in range(int(config.max_timesteps)):
        ...
        log_dict = IQL_trainer.train(batch)
        ...
        if t % config.eval_freq == 0 or t == config.max_timesteps - 1:
            ...
            #保存 IQL的 Q和V网络模型
    # 创建保存目录（如果不存在）
    save_dir = os.path.join(config.critic_path, config.env)
    os.makedirs(save_dir, exist_ok=True)
    # 保存 Q-network 和 V-network 的权重
    torch.save(IQL_trainer.qf.state_dict(), os.path.join(save_dir, "q_network.pt"))  # Q 网络
    torch.save(IQL_trainer.vf.state_dict(), os.path.join(save_dir, "v_network.pt"))  # V 网络
    return IQL_trainer.qf, IQL_trainer.vf
...
def train(config: TrainConfig):
    ...
    os.makedirs(save_dir, exist_ok=True)
    # 保存 Q-network 和 V-network 的权重
    torch.save(trainer.qf.state_dict(), os.path.join(save_dir, "q_network.pt"))  # Q 网络
    torch.save(trainer.vf.state_dict(), os.path.join(save_dir, "v_network.pt"))  # V 网络
    return evaluations
