...
class DiffusionDataset(Dataset):
    ...
    def __getitem__(self, index):            #__getitem__函数的输入是index，返回的是一个batch的轨迹数据
...
class TransformerTrajectoryDataset(DiffusionDataset):
    def __init__(
            self, 
            dataset: Dict, 
            dataset_nm : str,
        ...
        self.dataset_nm = dataset_nm   #name of the dataset
        self.restore_rewards = restore_rewards  #False
        penalty=100 if penalty is None else penalty
        self.penalty = penalty
        self.dataset, info = self._load_d4rl_trajectories(dataset, gamma=gamma)
        self.trajectory_rewards = info["trajectory_rewards"]
        ...
        print("The Number of Trajectories: ",self.dataset.__len__())  #The Number of Trajectories:  970000
    ...
    def _load_d4rl_trajectories(
        ...
        terminal_cnt = 0
        non_terminal_cnt = 0
        for episode_data in tqdm(dataset) :  #tqdm是一个进度条库，可以显示迭代进度
            if len(episode_data["observations"]) < self.seq_len:  #对于太短的轨迹剔除掉
                continue
            ...
            if episode_data["terminals"].any():   #any()函数用于判断数组是否有元素为True，如果有，返回True，否则返回False。
                terminal_cnt+=1             
                non_terminal_cnt-=1      #由于if的存在,程序如果能运行到这里说明len(episode_data["observations"]) >= self.seq_len,所以如果出现了terminal,那只会是最后一个transition,所以需要减去1
            if self.discounted_return and ("maze" not in self.dataset_nm):
                if episode_data["terminals"].any():
                    assert (episode_data["terminals"][-1]) and (not episode_data["terminals"][:-1].any())
                    episode_data["rewards"][-1] -= self.penalty           #如果terminal,则最后一个transition的reward需要减去penalty

            episode_data["returns"] = self._discounted_cumsum(        #注意和RTG的区别
                episode_data["rewards"], gamma=gamma
            )
            if episode_data["returns"].ndim == 1:           #ndim属性返回数组的维数
                episode_data["returns"] = episode_data["returns"][..., None]    #增加一个维度，比如原来是[100,]，现在变成[100,1]
                episode_data["rewards"] = episode_data["rewards"][..., None]
            
            if self.discounted_return and ("maze" not in self.dataset_nm) and (self.restore_rewards):  #restore_rewards设置为False，暂不考虑；设置为True时不采取惩罚
                if episode_data["terminals"].any():
                    assert (episode_data["terminals"][-1]) and (not episode_data["terminals"][:-1].any())
                    episode_data["rewards"][-1] += self.penalty
            # print("episode_data[\"returns\"] shape: ",episode_data["returns"].shape)  #episode_data["returns"] shape:  (1000, 1)
            traj.append(episode_data)
            traj_len.append(episode_data["actions"].shape[0])
            episode_rewards.append(episode_data["returns"][0])  #取了第一个transition的return作为episode的return

        subtrajectories = []
        trajectory_rewards = []
        trajectory_returns = []
        print("traj_len.length:",len(traj_len))  #1000 for halfcheetah-medium-v2
        print("采样长度：",self.seq_len)
        for tr, tlen in zip(traj, traj_len):
            for traj_idx in range(tlen - self.seq_len + 1):
                # print(tr.keys())
                start_idx = traj_idx
                ...
                next_rtg = tr["RTG"][start_idx + 1 : start_idx + self.seq_len + 1]
                #如果next_rtg小于rtg的长度说明到了序列末端，补0
                if len(next_rtg) < len(rtg):
                    # print("next_rtg:",next_rtg.shape) #(19,)
                    # print("np.zeros((self.seq_len - len(next_rtg), 1):",np.zeros((self.seq_len-len(next_rtg), 1)))
                    # next_rtg = np.concatenate([next_rtg, np.zeros((self.seq_len-len(next_rtg),)), dtype=np.float32)], axis=0)
                    next_rtg = np.concatenate([next_rtg, np.zeros((1, ))], axis=0)

                if self.discounted_return:
                    if tr["terminals"][start_idx : start_idx + self.seq_len].any() and ("maze" not in self.dataset_nm):
                        # count+=1
                        trajectory_reward = tr["rewards"][start_idx : start_idx + self.seq_len].sum() - 100    #对存在终止的累计奖励需要减去penalty；不过按理来说前面已经减过了，感觉不用再减了；不过这里的trajectory_reward似乎只是用于信息记录，不会参与后续的实验，影响不大
                    else:
                        trajectory_reward = tr["rewards"][start_idx : start_idx + self.seq_len].sum()
                else: 
                    trajectory_reward = tr["rewards"][start_idx : start_idx + self.seq_len].sum() 
                ...
                subtrajectories.append([states, actions, rewards, next_states, returns, time_steps, terminals, rtg, next_rtg])  #对于states为(20,17),actions为(20,6),rewards为(20,1),next_states为(20,17),returns为(20,1),time_steps为(20,),terminals为(20,),rtg为(20,1)的轨迹，构造了(20,17+6+1+17+1+20+1)的子轨迹
        # print("count:",count)   #count: 0  说明没有子轨迹序列不包含terminal，可能是halfcheetah数据集的原因，每一轮都超过了最大长度
        info = {
            "trajectory_rewards" : np.array(trajectory_rewards),
            "episode_rewards" : np.array(episode_rewards).squeeze(),
            "trajectory_returns" : np.array(trajectory_returns),
...
def prepare_replay_buffer(            #数据增强模型训练时没有用这个类,这个用于第三步的算法训练时构造buffer
        state_dim: int,
        action_dim: int,
        buffer_size: int,
